"""
Registration: xxxx; 
Description: Heun's Method 2nd Order ODE : Damped SHM d2x/dt2 + lambda*dx/dt + kx = 0 
...
if(solve):
   ...
   if  (dt==0.01):   fp = open("data/eulerPC_ode2_dt0.01.dat","w") 
   elif(dt==0.005):  fp = open("data/eulerPC_ode2_dt0.005.dat","w") 
   elif(dt==0.001):  fp = open("data/eulerPC_ode2_dt0.001.dat","w") 
   elif(dt==0.0005): fp = open("data/eulerPC_ode2_dt0.0005.dat","w")
   ...
   for i in range(step):
       ...
       [dxpdt, dypdt] = dshm(xp,yp,t,k,lam)
       xc = x + 0.5*dt*(dxdt+dxpdt)
       yc = y + 0.5*dt*(dydt+dypdt)
       t = tp
       x = xc
       y = yc
...
if(plot):
   ...
   # Read the datafiles
   fp1 = np.loadtxt('data/eulerPC_ode2_dt0.01.dat');   T1 = fp1[:,0]; X1 = fp1[:,1]; Y1 = fp1[:,2]
   fp2 = np.loadtxt('data/eulerPC_ode2_dt0.005.dat');  T2 = fp2[:,0]; X2 = fp2[:,1]; Y2 = fp2[:,2] 
   fp3 = np.loadtxt('data/eulerPC_ode2_dt0.001.dat');  T3 = fp3[:,0]; X3 = fp3[:,1]; Y3 = fp3[:,2] 
   fp4 = np.loadtxt('data/eulerPC_ode2_dt0.0005.dat'); T4 = fp4[:,0]; X4 = fp4[:,1]; Y4 = fp4[:,2]
   ...
   plt.yticks(size = 20)
   plt.grid()
   if(discr > 0):  plt.title('Weakly Damped Harmonic Motion (Heun)',fontsize=26)
   elif(discr==0): plt.title('Critically Damped Harmonic Motion (Heun)',fontsize=26)
   else:           plt.title('Heavily Damped Harmonic Motion (Heun)',fontsize=26)
